...
class Level3(Scene):
    def __init__(self, director, offset):
        ...
        self.world = World("Lvl3", self.enemies_group, self.randomEnemyFactory, self.interactiveGroup, self.cameraOffset, self.healthPickUps,self.destructibles_group, self.gunPickups, self.triggerGroup)
    ...
    def update(self, dt):
        ...
        for trigger in self.triggerGroup:
            ...
            if text != "":
                if text == "lvl2":
                    scene = lvl2.Level2(self.director, LVL3_TO_LVL2)
                    self.director.changeScene(scene)
                if text == "lvl2Vent":
                    scene = lvl2.Level2(self.director, LVL3_TO_LVL2_ALTER)
                    self.director.changeScene(scene)
