...
class Level3(level.Level):
    def __init__(self, director, offset, dificulty, player, uienergy, uipistol, uipistolUpgrade, uigrenadeLauncher, persist):
        super(Level3, self).__init__(director, offset, dificulty, player, uienergy, uipistol, uipistolUpgrade, uigrenadeLauncher, persist)
    ...
    def update(self, dt):
        ...
        for trigger in self.triggerGroup:
            ...
            if text != "":
                if text == "lvl2":
                    scene = lvl2.Level2(self.director, LVL3_TO_LVL2, self.dificulty, self.player, self.uiEnergy, self.uiPistol, self.uiPistolUpgrade, self.uiGrenadeLauncher, self.persist)
                    scene.startup()
                    self.director.changeScene(scene)
                if text == "lvl2Vent":
                    scene = lvl2.Level2(self.director, LVL3_TO_LVL2_ALTER, self.dificulty, self.player, self.uiEnergy, self.uiPistol, self.uiPistolUpgrade, self.uiGrenadeLauncher, self.persist)
                    scene.startup()
                    self.director.changeScene(scene)
