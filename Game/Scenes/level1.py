...
class Level1(level.Level):
    def __init__(self, director, offset, dificulty, player, uienergy, uicounter, healthBar, uicroshair, uipistol, uipistolUpgrade, uigrenadeLauncher):
        super(Level1, self).__init__(director, offset, dificulty, player, uienergy, uicounter, healthBar, uicroshair, uipistol, uipistolUpgrade, uigrenadeLauncher)
    ...
    def update(self, dt):
        ...
        for trigger in self.triggerGroup:
            ...
            if text != "":
                if text == "lvl2":
                    scene = lvl2.Level2(self.director, LVL1_TO_LVL2, self.dificulty, self.player, self.uiEnergy, self.uiCounter, self.healthBar, self.uiCroshair, self.uiPistol, self.uiPistolUpgrade, self.uiGrenadeLauncher)
                    scene.startup()
